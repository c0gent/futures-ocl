...
///
/// This variant used for testing purposes. `SubBufferPool` is the more useful
/// version.
///
...
impl<T: OclPrm> BufferPool<T> {
    ...
    pub fn new(len: u32, default_queue: Queue) -> BufferPool<T> {
        ...
        // let buffer = Buffer::<T>::new(default_queue.clone(), flags, len, None).unwrap();
            ...
            origin: u32, len: u32) -> usize
    {
        ...
        let buf = Buffer::new(self.default_queue.clone(), flags, region.len, None).unwrap();
        if let Some(idx) = self.sub_buffers.insert(region.buffer_id, buf) {
            panic!("Duplicate indexes: {}", idx); }
